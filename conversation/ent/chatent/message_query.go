...
func (mq *MessageQuery) QuerySpouse() *MessageQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2O, false, message.SpouseTable, message.SpouseColumn),
		)
...
func (mq *MessageQuery) QuerySession() *SessionQuery {
	query := (&SessionClient{config: mq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (mq *MessageQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("chatent: check existence: %w", err)
	default:
		return true, nil
	}
}
...
func (mq *MessageQuery) Clone() *MessageQuery {
	...
	return &MessageQuery{
		...
		withSpouse:  mq.withSpouse.Clone(),
		withSession: mq.withSession.Clone(),
...
func (mq *MessageQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range mq.inters {
		if inter == nil {
			return fmt.Errorf("chatent: uninitialized interceptor (forgotten import chatent/runtime?)")
		}
	...
	for _, f := range mq.ctx.Fields {
		if !message.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("chatent: invalid field %q for query", f)}
		}
	}
	if mq.path != nil {
...
func (mq *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	...
	if query := mq.withSpouse; query != nil {
		if err := mq.loadSpouse(ctx, query, nodes, nil,
			func(n *Message, e *Message) { n.Edges.Spouse = e }); err != nil {
			return nil, err
		}
	}
	if query := mq.withSession; query != nil {
		if err := mq.loadSession(ctx, query, nodes, nil,
...
func (mq *MessageQuery) loadSpouse(ctx context.Context, query *MessageQuery, nodes []*Message, init func(*Message), assign func(*Message, *Message)) error {
...
func (mq *MessageQuery) loadSession(ctx context.Context, query *SessionQuery, nodes []*Message, init func(*Message), assign func(*Message, *Session)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Message)
	for i := range nodes {
